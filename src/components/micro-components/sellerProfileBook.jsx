    ...
    function handleDelete() {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            fetch(`https://api-gateway-pearl.vercel.app/eliminar/${book.id}`, {
                method: 'DELETE',
                headers: {
                ...
                if (response.status === 400) {
                    Swal.fire('Error', 'No se encontró el auto', 'error');
                }
            });
            }
      ...
      return (
        <div data-aos="fade-up" id="book_for_sale_{id}" className="portfolio-item">
            <div className="portfolio-wrap">
                <button className="btn-edit" data-book-id="1" onClick={handleEdit}><FontAwesomeIcon icon={faFilePen} /></button>
                <button className="btn-delete" data-book-id="1" onClick={handleDelete}><FontAwesomeIcon icon={faTrash} /></button>  
                <img src={book.image.length > 0? book.image: urlImage} className="img-fluid" alt=""></img>
